...
public class MultiCometGradient extends CustomizableGradient implements DimensionListener{

	
		
		protected ArrayList<CustomCometGradientPaint> ps = new ArrayList<>();
		...
		public synchronized void paintGradientColor(Graphics2D g2, int amountFull,float speed, Color darkColor, Color lightColor){
	      ...
	      if (newGap<0){
	    	  newGap = (int) (gap*(0.5 + Math.random()));	    	  
	  		  CometGradient com = new CometGradient();
	  		  com.init(bar);
		      ...
		      ps.add(new CustomCometGradientPaint(com,0, darkColor,lightColor,(float)((r-1) * 1.0/row),(float)(r*1.0/row)));
	  	  }
